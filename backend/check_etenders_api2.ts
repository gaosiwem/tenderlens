...
async function main() {
  const res = await fetch(ETENDERS_DEFAULT_URL, {
    method: "GET",
    headers: {
  ...
  const withMany = rows.filter((r: any) => r.supportDocument && r.supportDocument.length > 1);
  console.log(`Rows with >1 support docs: ${withMany.length}`);
  if (withMany.length > 0) {
     fs.writeFileSync('output.json', JSON.stringify(withMany[0].supportDocument, null, 2));
